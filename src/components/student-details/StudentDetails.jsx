...
export default function StudentDetails() {
        ...
        return (
            <>
                <Box sx={{ pt: 5, pb: 2 }}>
                    ...
                    <span className='student-details--score-status' style={{ backgroundColor: scoreStatus[subject.status].color }}>{scoreStatus[subject.status].label}</span>
                </Box>
                <Box sx={{ display: 'flex' }}>
                    {
                        subject.elements.map(ele => (
                            <Box sx={{ boxShadow: '0 0 5px rgba(0, 0, 0, 0.5)' }} className={'student-details--score-card'}>
                                <Box sx={{ backgroundColor: '#2d3748', color: '#FFF', textAlign: 'center', padding: '5px 0' }}>
                                    <p>{ele.name}</p>
                                </Box>
                                <Box sx={{ display: 'flex' }}>
                                    {
                                        ele.parts.map(part => (
                                            <div className={`student-details--score-card-tile`}>
                                                <p>{part.name}</p>
                                                <p className='student-details--score-card-tile-score'>{part.score}</p>
                                            </div>
                                        ))
                                    }
                                    <div className={`student-details--score-card-tile`} style={{ backgroundColor: '#dddcdc' }}>
                                        <p>Avg.</p>
                                        <p className='student-details--score-card-tile-score'>{calculateAverage(ele.parts.map(s => s.score))}</p>
                                    </div>
                                </Box>
                            </Box>
                        ))
    ...
    const ScoreContainer = () => {
        return (
            <Box sx={{ fontFamily: 'Arial, sans-serif', padding: '20px 2vw' }}>
                 <Typography variant={'h4'} component={'h6'} style={{
    ...
    return (
        <>
            ...
            <Box>
                <Box className='student-details--label'>
                    General
                </Box>
                <Box sx={{ margin: '30px 25px' }}>
                    <Grid container>
                        <Grid item md={6}>
                            ...
                            <div className="student-details--info-field">
                                <span className="student-details--info-label">Date of birth</span>
                        ...
                        <Grid item md={6}>
                            ...
                            <div className="student-details--info-field">
                                <span className="student-details--info-label">Permanent residence</span>
                                <span>AAA</span>
                            </div>
                            <div className="student-details--info-field">
                            ...
                            <div className="student-details--info-field">
                                <span className="student-details--info-label">Certification status</span>
                                <span>Done</span>
                            </div>
                            <div className="student-details--info-field">
                                <span className="student-details--info-label">Certification date</span>
